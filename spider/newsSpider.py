...
http_response = getResponse("http://www.chinawenben.com/file/esswr6xuxzastvocstatu3ws_1.html") #拿到http请求后的上下文对象(HTTPResponse object)
data = http_response.read().decode('utf-8')
p= r'<p>+[^"]+</p>'
t=r'[^\[\'][^<p>]+[^</p>]'
test = str(re.findall(p,data))
print (test)
test2=re.findall(t,test)
